...
const getImg = async () => {
  ...
  const res = await fetch(`https://movies-kux5.onrender.com/search/${query.value}`);
  if (!res.ok) {
...
const getTopRated = async () => {
  isFetching.value = true;
  const res = await fetch("https://movies-kux5.onrender.com/top-rated");
  if (!res.ok) {
...
const nowPlaying = async () => {
  isFetching.value = true;
  const res = await fetch("https://movies-kux5.onrender.com/now");
  if (!res.ok) {
    not_connected.value = true;
    throw new Error("Error fetching data");
  }
...
const getMostPopular = async () => {
  isFetching.value = true;
  const res = await fetch("https://movies-kux5.onrender.com/popular");
  if (!res.ok) {
    not_connected.value = true;
    throw new Error("Error fetching data");
  }
